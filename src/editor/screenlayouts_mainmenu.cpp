...
#include "screenlayouts_mainmenu.h"
#include "storagefile.h"

ScreenLayouts_MainMenu::ScreenLayouts_MainMenu(QWidget *parent) : QWidget(parent)
{
	setupUi(this);
	mainMenuScene = new QGraphicsScene(0, 0, 640, 480);
	gvMainMenuLayout->setScene(mainMenuScene);
	gvMainMenuLayout->setBackgroundBrush(QBrush(QImage(":/icons/graphicsview_background.png")));
...
ScreenLayouts_MainMenu::~ScreenLayouts_MainMenu()
{
	if (layoutRef != NULL)
	{
		delete layoutRef;
...
void ScreenLayouts_MainMenu::setupLayout()
{
	layoutRef = NULL;
	...
	mainMenuScene->addItem(menuBackgroundItem);
	...
	mainMenuScene->addItem(menuGridItem);
	...
	mainMenuScene->addItem(resizeItem_topLeft);
	...
	mainMenuScene->addItem(resizeItem_top);
	...
	mainMenuScene->addItem(resizeItem_topRight);
	...
	mainMenuScene->addItem(resizeItem_left);
	...
	mainMenuScene->addItem(resizeItem_right);
	...
	mainMenuScene->addItem(resizeItem_bottomLeft);
	...
	mainMenuScene->addItem(resizeItem_bottom);
	...
	mainMenuScene->addItem(resizeItem_bottomRight);
	...
	mainMenuScene->addItem(selectionRect);
	...
	mainMenuScene->addItem(highlightedObject);
}

void ScreenLayouts_MainMenu::shutdownLayout()
{
	if (layoutRef != NULL)
	{
		delete layoutRef;
...
void ScreenLayouts_MainMenu::unloadCurrentLayout()
{
	if (layoutRef != NULL)
	{
		delete layoutRef;
...
void ScreenLayouts_MainMenu::setCurrentLayout(int layoutID)
{
	ignoreEvents = true;
	if (layoutRef != NULL)
	{
	...
	clearGraphicsScene();
	twObjectList->clear();
	layoutRef = new ProjectData::DataReference<MainMenuLayout>(layoutID, __FILE__, __LINE__);
	(*layoutRef)->setGraphicsScene(mainMenuScene);
...
void ScreenLayouts_MainMenu::changeCurrentLayoutsName(QString newName)
{
	(*layoutRef)->setName(newName);
	ProjectData::mainMenuLayoutNames[layoutRef->getDataID()] = newName;
}

void ScreenLayouts_MainMenu::on_aCopy_triggered()
{
...
void ScreenLayouts_MainMenu::on_aCut_triggered()
{
...
void ScreenLayouts_MainMenu::on_aDeleteItem_triggered()
{
	MenuObject *objectToDelete = selectedObject;
...
void ScreenLayouts_MainMenu::on_aPaste_triggered()
{
...
void ScreenLayouts_MainMenu::on_bPreview_clicked()
{
	MenuPreview *preview = new MenuPreview(this);
	preview->setupPreview(layoutRef->getDataID(), MenuPreview::MENUTYPE_MAINMENU);
	preview->exec();
	delete preview;
}

void ScreenLayouts_MainMenu::on_cboxZoom_currentIndexChanged(int index)
{
	gvMainMenuLayout->resetMatrix();
	lwMenuItems->setScaleFactor(1.0);
	if (index == ZOOM_25)
	{
		gvMainMenuLayout->scale(.25, .25);
		lwMenuItems->setScaleFactor(.25);
	}
	else if (index == ZOOM_50)
	{
		gvMainMenuLayout->scale(.5, .5);
		lwMenuItems->setScaleFactor(.5);
	}
	else if (index == ZOOM_75)
	{
		gvMainMenuLayout->scale(.75, .75);
		lwMenuItems->setScaleFactor(.75);
	}
	else if (index == ZOOM_150)
	{
		gvMainMenuLayout->scale(1.5, 1.5);
		lwMenuItems->setScaleFactor(1.5);
	}
	else if (index == ZOOM_200)
	{
		gvMainMenuLayout->scale(2.0, 2.0);
		lwMenuItems->setScaleFactor(2.0);
	}
	else if (index == ZOOM_FITTOWINDOW)
	{
		float xScale = (float)gvMainMenuLayout->width() / 640.0;
		float yScale = (float)gvMainMenuLayout->height() / 480.0;
		if (xScale > yScale)
		{
			gvMainMenuLayout->scale(yScale, yScale);
			lwMenuItems->setScaleFactor(yScale);
		}
		else
		{
			gvMainMenuLayout->scale(xScale, xScale);
			lwMenuItems->setScaleFactor(xScale);
		}
	}
}

void ScreenLayouts_MainMenu::on_gvMainMenuLayout_customContextMenuRequested(const QPoint &pos)
{
	MenuObject *object;
	QPointF scenePos = gvMainMenuLayout->mapToScene(pos);
	QPoint globalPos = gvMainMenuLayout->mapToGlobal(pos);
...
void ScreenLayouts_MainMenu::on_gvMainMenuLayout_dragEnterEvent(QDragEnterEvent *event)
{
	if (event->source() == lwMenuItems && layoutRef != NULL)
		event->acceptProposedAction();
}

void ScreenLayouts_MainMenu::on_gvMainMenuLayout_dragLeaveEvent(QDragLeaveEvent *event)
{
	Q_UNUSED(event);
	highlightedObject->setVisible(false);
}

void ScreenLayouts_MainMenu::on_gvMainMenuLayout_dragMoveEvent(QDragMoveEvent *event)
{
	if (event->source() == lwMenuItems && layoutRef != NULL)
	{
		MenuObject *parentObject, *tempObject;
		QGraphicsPixmapItem *pixmapItem;
		QPointF scenePos = gvMainMenuLayout->mapToScene(event->pos());
...
void ScreenLayouts_MainMenu::on_gvMainMenuLayout_dropEvent(QDropEvent *event)
{
	highlightedObject->setVisible(false);
	if (event->source() == lwMenuItems && layoutRef != NULL)
	{
...
void ScreenLayouts_MainMenu::on_gvMainMenuLayout_leaveEvent(QEvent *event)
{
	Q_UNUSED(event);
	QApplication::restoreOverrideCursor();
}

void ScreenLayouts_MainMenu::on_gvMainMenuLayout_mouseDoubleClickEvent(QMouseEvent *event)
{
	Q_UNUSED(event);
	if (selectedObject != NULL)
	{
...
void ScreenLayouts_MainMenu::on_gvMainMenuLayout_mouseMoveEvent(QMouseEvent *event)
{
	QPointF scenePos = gvMainMenuLayout->mapToScene(event->pos());
	QPoint globalPos = gvMainMenuLayout->mapToGlobal(event->pos());
	QString toolTip = "";
...
void ScreenLayouts_MainMenu::on_gvMainMenuLayout_mousePressEvent(QMouseEvent *event)
{
	QPointF scenePos = gvMainMenuLayout->mapToScene(event->pos());
	QPoint globalPos = gvMainMenuLayout->mapToGlobal(event->pos());
	hotspot = scenePos;
...
void ScreenLayouts_MainMenu::on_gvMainMenuLayout_mouseReleaseEvent(QMouseEvent *event)
{
	int snapAmount, gridSize = cboxGridSize->currentIndex();
	if (cbSnapToGrid->isChecked())
	{
	...
	QToolTip::showText(gvMainMenuLayout->mapToGlobal(event->pos()), "");
	resizeInProgress = false;
	repositionInProgress = false;
	ignoreEvents = false;
	selectedObjectChanged();
...
void ScreenLayouts_MainMenu::on_twObjectList_customContextMenuRequested(const QPoint &pos)
{
...
void ScreenLayouts_MainMenu::on_twObjectList_itemSelectionChanged()
{
	if (!ignoreEvents)
	{
		...
		selectedObjectChanged();
		if (selectedObject != NULL)
			gvMainMenuLayout->ensureVisible(selectedObject->getPixmapItem());
	}
}

void ScreenLayouts_MainMenu::on_twObjectList_itemDoubleClicked(QTreeWidgetItem *item, int column)
{
	if (column == 0 && selectedObject != NULL)
		twObjectList->editItem(item, column);
}

void ScreenLayouts_MainMenu::on_twObjectList_itemChanged(QTreeWidgetItem *item, int column)
{
...
void ScreenLayouts_MainMenu::on_twProperties_itemChanged(QTableWidgetItem *item)
{
	if (!ignoreEvents)
	{
...
void ScreenLayouts_MainMenu::on_bEasyNavigationCreator_clicked()
{
...
void ScreenLayouts_MainMenu::on_bEditMenusEvents_clicked()
{
	MenuEventEditor *editor = new MenuEventEditor(layoutRef->getDataID(), MenuObject::MENUTYPE_MAINMENU);
	editor->exec();
	delete editor;
}

void ScreenLayouts_MainMenu::on_cbSnapToGrid_toggled(bool checked)
{
	wSnapToGrid->setEnabled(checked);
	menuGridItem->setVisible(checked);
}

void ScreenLayouts_MainMenu::on_cboxGridSize_currentIndexChanged(int index)
{
	if (index == SIZE_2x2)
		menuGridItem->setBrush(QBrush(QImage(":/images/menugrid_2x2.png")));
...
void ScreenLayouts_MainMenu::applyMenuNavigation()
{
	(*layoutRef)->applyMenuNavigation();
}

void ScreenLayouts_MainMenu::selectedObjectChanged()
{
	if (selectedObject != NULL)
	{
...
MenuObject *ScreenLayouts_MainMenu::getObjectUnderCursor(QPointF scenePos)
{
...
void ScreenLayouts_MainMenu::buildObjectTree(MenuObject *parentObject, QTreeWidgetItem *parentItem)
{
...
void ScreenLayouts_MainMenu::deleteChildItems(QTreeWidgetItem *item)
{
...
void ScreenLayouts_MainMenu::addObjectToLayout(MenuObject *object)
{
...
void ScreenLayouts_MainMenu::clearGraphicsScene()
{
	mainMenuScene->clear();
	menuBackgroundItem = new QGraphicsRectItem(0, 0, 640, 480);
	menuBackgroundItem->setPen(Qt::NoPen);
