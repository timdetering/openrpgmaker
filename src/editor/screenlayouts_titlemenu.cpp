...
#include "screenlayouts_titlemenu.h"
#include "storagefile.h"

ScreenLayouts_TitleMenu::ScreenLayouts_TitleMenu(QWidget *parent) : QWidget(parent)
{
	setupUi(this);
	titleMenuScene = new QGraphicsScene(0, 0, 640, 480);
	gvTitleMenuLayout->setScene(titleMenuScene);
	gvTitleMenuLayout->setBackgroundBrush(QBrush(QImage(":/icons/graphicsview_background.png")));
...
ScreenLayouts_TitleMenu::~ScreenLayouts_TitleMenu()
{
	layout = NULL;
}

void ScreenLayouts_TitleMenu::setupLayout()
{
	ignoreEvents = true;
	menuBackgroundItem = new QGraphicsPixmapItem(QPixmap(ProjectData::titleLocation));
	menuBackgroundItem->setPos(0.0, 0.0);
	menuBackgroundItem->setZValue(-2);
	if (ProjectData::titleLocation.isEmpty())
		menuBackgroundItem->setVisible(false);
	titleMenuScene->addItem(menuBackgroundItem);
	backupMenuBackgroundItem = new QGraphicsRectItem(0, 0, 640, 480);
	backupMenuBackgroundItem->setPen(Qt::NoPen);
	if (ProjectData::systemGraphics.stretchToFit)
		backupMenuBackgroundItem->setBrush(QBrush(QImage(ProjectData::systemGraphics.menuBGLocation).scaled(640, 480)));
	else
		backupMenuBackgroundItem->setBrush(QBrush(QImage(ProjectData::systemGraphics.menuBGLocation)));
	backupMenuBackgroundItem->setZValue(-3);
	titleMenuScene->addItem(backupMenuBackgroundItem);
	...
	titleMenuScene->addItem(menuGridItem);
	...
	titleMenuScene->addItem(resizeItem_topLeft);
	...
	titleMenuScene->addItem(resizeItem_top);
	...
	titleMenuScene->addItem(resizeItem_topRight);
	...
	titleMenuScene->addItem(resizeItem_left);
	...
	titleMenuScene->addItem(resizeItem_right);
	...
	titleMenuScene->addItem(resizeItem_bottomLeft);
	...
	titleMenuScene->addItem(resizeItem_bottom);
	...
	titleMenuScene->addItem(resizeItem_bottomRight);
	...
	titleMenuScene->addItem(selectionRect);
	...
	titleMenuScene->addItem(highlightedObject);
	layout = ProjectData::titleMenuLayout;
	layout->setGraphicsScene(titleMenuScene);
	...
	for (int i = 0; i < layout->numTopLevelObjects(); ++i)
	{
		...
		MenuObject *object = layout->getTopLevelObject(i);
	...
	layout->updateMenuObjects();
	ignoreEvents = false;
}

void ScreenLayouts_TitleMenu::shutdownLayout()
{
	ProjectData::saveTitleMenuLayoutToStorage();
	layout->unsetGraphicsScene();
	titleMenuScene->clear();
	ignoreEvents = true;
	twObjectList->clear();
...
void ScreenLayouts_TitleMenu::on_aCopy_triggered()
{
	if (selectedObject != NULL)
	{
		MenuObject *copyObject = NULL;
...
void ScreenLayouts_TitleMenu::on_aCut_triggered()
{
...
void ScreenLayouts_TitleMenu::on_aDeleteItem_triggered()
{
	MenuObject *objectToDelete = selectedObject;
...
void ScreenLayouts_TitleMenu::on_aPaste_triggered()
{
...
void ScreenLayouts_TitleMenu::on_bPreview_clicked()
{
	MenuPreview *preview = new MenuPreview(this);
	preview->setupPreview(0, MenuPreview::MENUTYPE_MAINMENU);
	preview->exec();
	delete preview;
}

void ScreenLayouts_TitleMenu::on_cboxZoom_currentIndexChanged(int index)
{
	gvTitleMenuLayout->resetMatrix();
	lwMenuItems->setScaleFactor(1.0);
	if (index == ZOOM_25)
	{
		gvTitleMenuLayout->scale(.25, .25);
		lwMenuItems->setScaleFactor(.25);
	}
	else if (index == ZOOM_50)
	{
		gvTitleMenuLayout->scale(.5, .5);
		lwMenuItems->setScaleFactor(.5);
	}
	else if (index == ZOOM_75)
	{
		gvTitleMenuLayout->scale(.75, .75);
		lwMenuItems->setScaleFactor(.75);
	}
	else if (index == ZOOM_150)
	{
		gvTitleMenuLayout->scale(1.5, 1.5);
		lwMenuItems->setScaleFactor(1.5);
	}
	else if (index == ZOOM_200)
	{
		gvTitleMenuLayout->scale(2.0, 2.0);
		lwMenuItems->setScaleFactor(2.0);
	}
	else if (index == ZOOM_FITTOWINDOW)
	{
		float xScale = (float)gvTitleMenuLayout->width() / 640.0;
		float yScale = (float)gvTitleMenuLayout->height() / 480.0;
		if (xScale > yScale)
		{
			gvTitleMenuLayout->scale(yScale, yScale);
			lwMenuItems->setScaleFactor(yScale);
		}
		else
		{
			gvTitleMenuLayout->scale(xScale, xScale);
			lwMenuItems->setScaleFactor(xScale);
		}
	}
}

void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_customContextMenuRequested(const QPoint &pos)
{
	MenuObject *object;
	QPointF scenePos = gvTitleMenuLayout->mapToScene(pos);
	QPoint globalPos = gvTitleMenuLayout->mapToGlobal(pos);
...
void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_dragEnterEvent(QDragEnterEvent *event)
{
	if (event->source() == lwMenuItems && layout != NULL)
		event->acceptProposedAction();
}

void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_dragLeaveEvent(QDragLeaveEvent *event)
{
	Q_UNUSED(event);
	highlightedObject->setVisible(false);
}

void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_dragMoveEvent(QDragMoveEvent *event)
{
	if (event->source() == lwMenuItems && layout != NULL)
	{
		MenuObject *parentObject, *tempObject;
		QGraphicsPixmapItem *pixmapItem;
		QPointF scenePos = gvTitleMenuLayout->mapToScene(event->pos());
...
void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_dropEvent(QDropEvent *event)
{
	highlightedObject->setVisible(false);
	if (event->source() == lwMenuItems && layout != NULL)
	{
		...
		if (parentObject != NULL)
			...
			droppedObject->setLayout(layout);
		objectItem = new QTreeWidgetItem;
...
void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_leaveEvent(QEvent *event)
{
	Q_UNUSED(event);
	QApplication::restoreOverrideCursor();
}

void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_mouseDoubleClickEvent(QMouseEvent *event)
{
	Q_UNUSED(event);
	if (selectedObject != NULL)
	{
...
void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_mouseMoveEvent(QMouseEvent *event)
{
	QPointF scenePos = gvTitleMenuLayout->mapToScene(event->pos());
	QPoint globalPos = gvTitleMenuLayout->mapToGlobal(event->pos());
	QString toolTip = "";
	int offsetX = (int)(hotspot.x() - scenePos.x());
	int offsetY = (int)(hotspot.y() - scenePos.y());
	int snapAmount = 1, gridSize = cboxGridSize->currentIndex();
...
void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_mousePressEvent(QMouseEvent *event)
{
	QPointF scenePos = gvTitleMenuLayout->mapToScene(event->pos());
	QPoint globalPos = gvTitleMenuLayout->mapToGlobal(event->pos());
	hotspot = scenePos;
		...
		if (resizeItem_topLeft->isVisible())
		{
			...
			else
			{
				...
				if (selectedObject != NULL)
				{
					...
					layout->removeTopLevelObject(selectedObject);
					titleMenuScene->addItem(item);
					hotspot -= itemPos;
				}
				else
					repositionInProgress = false;
...
void ScreenLayouts_TitleMenu::on_gvTitleMenuLayout_mouseReleaseEvent(QMouseEvent *event)
{
	int snapAmount = 1, gridSize = cboxGridSize->currentIndex();
	if (cbSnapToGrid->isChecked())
	{
	...
	QToolTip::showText(gvTitleMenuLayout->mapToGlobal(event->pos()), "");
	resizeInProgress = false;
	repositionInProgress = false;
	ignoreEvents = false;
	selectedObjectChanged();
...
void ScreenLayouts_TitleMenu::on_twObjectList_customContextMenuRequested(const QPoint &pos)
{
...
void ScreenLayouts_TitleMenu::on_twObjectList_itemSelectionChanged()
{
	if (!ignoreEvents)
	{
		QTreeWidgetItem *item = twObjectList->currentItem();
		if (item != NULL)
			selectedObject = layout->getObjectByName(item->text(0));
		else
			selectedObject = NULL;
		selectedObjectChanged();
		if (selectedObject != NULL)
			gvTitleMenuLayout->ensureVisible(selectedObject->getPixmapItem());
	}
}

void ScreenLayouts_TitleMenu::on_twObjectList_itemDoubleClicked(QTreeWidgetItem *item, int column)
{
	if (column == 0 && selectedObject != NULL)
		twObjectList->editItem(item, column);
}

void ScreenLayouts_TitleMenu::on_twObjectList_itemChanged(QTreeWidgetItem *item, int column)
{
...
void ScreenLayouts_TitleMenu::on_twProperties_itemChanged(QTableWidgetItem *item)
{
	if (!ignoreEvents)
	{
...
void ScreenLayouts_TitleMenu::on_bEasyNavigationCreator_clicked()
{
	MenuNavigationCreator *navCreator = new MenuNavigationCreator(layout->getObjectNamesByType(MenuObject::TYPE_SELECTABLEAREA), layout->getNavigation(), this);
...
void ScreenLayouts_TitleMenu::on_bEditMenusEvents_clicked()
{
	MenuEventEditor *editor = new MenuEventEditor(0, MenuObject::MENUTYPE_TITLESCREENMENU);
	editor->exec();
	delete editor;
}

void ScreenLayouts_TitleMenu::on_cbSnapToGrid_toggled(bool checked)
{
	wSnapToGrid->setEnabled(checked);
	menuGridItem->setVisible(checked);
}

void ScreenLayouts_TitleMenu::on_cboxGridSize_currentIndexChanged(int index)
{
	if (index == SIZE_2x2)
		menuGridItem->setBrush(QBrush(QImage(":/images/menugrid_2x2.png")));
...
void ScreenLayouts_TitleMenu::applyMenuNavigation()
{
	layout->applyMenuNavigation();
}

void ScreenLayouts_TitleMenu::selectedObjectChanged()
{
	if (selectedObject != NULL)
	{
...
MenuObject *ScreenLayouts_TitleMenu::getObjectUnderCursor(QPointF scenePos)
{
	QList<MenuObject*> objectList = layout->getTopLevelObjects();
...
void ScreenLayouts_TitleMenu::buildObjectTree(MenuObject *parentObject, QTreeWidgetItem *parentItem)
{
...
void ScreenLayouts_TitleMenu::deleteChildItems(QTreeWidgetItem *item)
{
...
void ScreenLayouts_TitleMenu::addObjectToLayout(MenuObject *object)
{
