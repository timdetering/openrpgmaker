...
BattleAnimation::BattleAnimation()
{
	...
	name = "Untitled";
	xAxisLine = POS_CENTER;
	yAxisLine = POS_MIDDLE;
	frameRate = 12;
}

BattleAnimation::BattleAnimation(BattleAnimation &other)
{
	...
	name = other.name;
	xAxisLine = other.xAxisLine;
	yAxisLine = other.yAxisLine;
...
BattleAnimation::BattleAnimation(XMLNode AnimationNode)
{
	XMLNode TempNode1, TempNode2, TempNode3;
	QString temp_string;
	name = AnimationNode.getAttribute("name");
	temp_string = AnimationNode.getAttribute("xAxisLine");
	if (temp_string == "left")
		xAxisLine = POS_LEFT;
		...
		xAxisLine = POS_RIGHT;
	temp_string = AnimationNode.getAttribute("yAxisLine");
	if (temp_string == "top")
		yAxisLine = POS_TOP;
	else if (temp_string == "middle")
		yAxisLine = POS_MIDDLE;
	else
		yAxisLine = POS_BOTTOM;
	...
	for (int i = 0, count = AnimationNode.nChildNode("frame"); i < count; ++i)
	{
		...
		for (int j = 0, count = TempNode1.nChildNode("effect"); j < count; ++j)
		{
			...
			temp_string = TempNode2.getAttribute("flashScope");
			if (temp_string == "target")
				effect->flashScope = SCOPE_TARGET;
			else if (temp_string == "screen")
				effect->flashScope = SCOPE_SCREEN;
			else
				effect->flashScope = SCOPE_NONE;
			effect->flashOpacity = QString(TempNode2.getAttribute("flashOpacity")).toInt();
			temp_string = TempNode2.getAttribute("shakeScope");
			if (temp_string == "target")
				effect->shakeScope = SCOPE_TARGET;
			else if (temp_string == "screen")
				effect->shakeScope = SCOPE_SCREEN;
			else
				effect->shakeScope = SCOPE_NONE;
...
BattleAnimation::BattleAnimation(StorageFile &storageFile)
{
	int count1, count2;
	int r, g, b;
	char *temp_string;
	...
	name = temp_string;
	delete[] temp_string;
	xAxisLine = storageFile.getSignedInt();
	yAxisLine = storageFile.getSignedInt();
...
void BattleAnimation::saveToStorageFile(StorageFile &storageFile)
{
	storageFile.putString(name.toUtf8().data());
	storageFile.putSignedInt(xAxisLine);
	storageFile.putSignedInt(yAxisLine);
...
QStringList BattleAnimation::getResources()
{
...
XMLNode BattleAnimation::getAnimationNode()
{
	XMLNode TempNode1, TempNode2, TempNode3, AnimationNode = XMLNode::createXMLTopNode("battleanimation");
	AnimationNode.addAttribute("name", name.toUtf8().data());
	if (xAxisLine == POS_LEFT)
		AnimationNode.addAttribute("xAxisLine", "left");
	else if (xAxisLine == POS_CENTER)
		AnimationNode.addAttribute("xAxisLine", "center");
	else
		AnimationNode.addAttribute("xAxisLine", "right");
	if (yAxisLine == POS_TOP)
		AnimationNode.addAttribute("yAxisLine", "top");
	else if (yAxisLine == POS_MIDDLE)
		AnimationNode.addAttribute("yAxisLine", "middle");
	else
		AnimationNode.addAttribute("yAxisLine", "bottom");
	...
	for (int i = 0; i < frames.size(); ++i)
	{
		...
		for (int j = 0; j < frame->animationEffects.size(); ++j)
		{
			...
			if (effect->flashScope == SCOPE_NONE)
				TempNode2.addAttribute("flashScope", "none");
			else if (effect->flashScope == SCOPE_TARGET)
				TempNode2.addAttribute("flashScope", "target");
			else if (effect->flashScope == SCOPE_SCREEN)
				TempNode2.addAttribute("flashScope", "screen");
			TempNode2.addAttribute("flashOpacity", QString::number(effect->flashOpacity).toUtf8().data());
			if (effect->shakeScope == SCOPE_NONE)
				TempNode2.addAttribute("shakeScope", "none");
			else if (effect->shakeScope == SCOPE_TARGET)
				TempNode2.addAttribute("shakeScope", "target");
			else if (effect->shakeScope == SCOPE_SCREEN)
				TempNode2.addAttribute("shakeScope", "screen");
...
BattleAnimation::AnimationFrame *BattleAnimation::getFrame(int frameNumber)
{
	return frames[frameNumber];
}

int BattleAnimation::numFrames()
{
	return frames.size();
}

void BattleAnimation::setNumFrames(int numFrames)
{
...
QString BattleAnimation::getImageLocation(int imageID)
{
	return images[imageID];
}

QStringList BattleAnimation::getImageLocations()
{
	return images;
}

int BattleAnimation::numImages()
{
	return images.size();
}

void BattleAnimation::addImage(QString imageLocation)
{
	images.append(imageLocation);
}

void BattleAnimation::removeImage(int imageID)
{
	for (int i = 0; i < frames.size(); ++i)
	{
		int j = 0;
...
QString BattleAnimation::getName()
{
	return name;
}

void BattleAnimation::setName(QString value)
{
	name = value;
}

int BattleAnimation::getXAxisLine()
{
	return xAxisLine;
}

void BattleAnimation::setXAxisLine(int pos)
{
	xAxisLine = pos;
}

int BattleAnimation::getYAxisLine()
{
	return yAxisLine;
}

void BattleAnimation::setYAxisLine(int pos)
{
	yAxisLine = pos;
}

int BattleAnimation::getFrameRate()
{
	return frameRate;
}

void BattleAnimation::setFrameRate(int rate)
{
	frameRate = rate;
}

bool BattleAnimation::tryLoad(XMLNode AnimationNode)
{
	XMLNode TempNode1, TempNode2, TempNode3;
	if (!XMLTools::attributeExists(AnimationNode, "name"))
		return false;
	if (!XMLTools::attributeExists(AnimationNode, "xAxisLine"))
		return false;
	if (!XMLTools::attributeStringValid(AnimationNode, "xAxisLine", QStringList() << "left" << "center" << "right", 2))
		return false;
	if (!XMLTools::attributeExists(AnimationNode, "yAxisLine"))
		return false;
	if (!XMLTools::attributeStringValid(AnimationNode, "yAxisLine", QStringList() << "top" << "middle" << "bottom", 2))
		return false;
	...
	for (int i = 0, count = AnimationNode.nChildNode("frame"); i < count; ++i)
	{
		...
		for (int j = 0, count = TempNode1.nChildNode("effect"); j < count; ++j)
		{
			...
			if (!XMLTools::attributeExists(TempNode2, "flashScope"))
				return false;
			if (!XMLTools::attributeStringValid(TempNode2, "flashScope", QStringList() << "none" << "target" << "screen", 0))
				return false;
			...
			if (!XMLTools::attributeExists(TempNode2, "shakeScope"))
				return false;
			if (!XMLTools::attributeStringValid(TempNode2, "shakeScope", QStringList() << "none" << "target" << "screen", 0))
				return false;
